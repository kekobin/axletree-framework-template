...
var debuglog = require('debuglog')('axletree/recv-reload');
var tar = require('tar');
var axletree = require('../../lib/axletree-kernel.js');
...
function reloadApp(appName) {
    ...
    appName = appName || '';
    var appPath = axletree.conf.dispatcher.appPath || path.join(axletree.ROOT_PATH, 'app');
    var appModulePath = path.join(appPath, appName);
    cleanCacheForFolder(appModulePath);
    if (axletree.dispatcher && axletree.dispatcher.cleanCache) {
        axletree.dispatcher.cleanCache();
        debuglog('clean dispatcher cache');
    }
    if (axletree.require && axletree.require.cleanCache) {
        axletree.require.cleanCache();
        debuglog('clean axletree require cache');
    }
}

function reloadIsomorphic() {
    if (axletree.plugins.isomorphic && axletree.plugins.isomorphic.cleanCache) {
        axletree.plugins.isomorphic.cleanCache();
        if (axletree.conf.static) {
            cleanCacheForFolder(axletree.conf.static.staticPath);
        } else {
            cleanCacheForFolder(path.join(axletree.ROOT_PATH, 'static'));
        }
...
function reloadView() {
    if (axletree.view && axletree.view.cleanCache) {
        axletree.view.cleanCache();
...
function cleanCacheForFolder(moduleFolderPath) {
    ...
    for (var i = 0; i < modules.length; i++) {
        ...
        if (modulePath.toLowerCase().indexOf(moduleFolderPath) === 0) {
            ...
            debuglog('clean cache: ', path.relative(axletree.ROOT_PATH, modulePath));
        }
    }
}
...
function extract(part, to, cb) {
    ...
    var extractStream = tar.Extract({
        path: path.join(axletree.ROOT_PATH, to),
        strip: 0
    });
    // 将文件流发送至解压流
...
module.exports['recv-reload'] = ['dispatcher',
    function (app, conf) {
        ...
        // only enable when axletree_DEBUG=true
        if (axletree.DEBUG) {
            ...
            app.post(conf.receiverUrl, function (req, res, next) {
                ...
                form.parse(req, function (err, fields, files) {
                    ...
                    if (fields.to) {
                        var appRootPath = axletree.conf.dispatcher.appPath || path.join(axletree.ROOT_PATH, 'app');
                        var deployPath = path.join(axletree.ROOT_PATH, fields.to.toString());
            ...
            axletree.reloadApp = reloadApp;
            axletree.reloadView = reloadView;
            axletree.reloadIsomorphic = reloadIsomorphic;
        }
...
module.exports['recv-reload'].defaultConf = {
    tarReceiverUrl: '/axletree/uploadtar',
    cleanCacheUrl: '/axletree/reload',
    receiverUrl: '/axletree/upload',
    uploadTimeout: 30,
    maxTarSize: '200mb',
    onCacheClean: null,
    lazyAppReload: false
